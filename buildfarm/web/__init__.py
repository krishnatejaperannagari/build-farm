...
class LogPrettyPrinter(object):
    ...
    def _pretty_print(self, m):
        ...
        if match:
           ...
           match = re.search("(passed)|(uxpassed)", m.group(3), re.I)
           if match:
    ...
    def _format_testsuite(self, m):
        ...
        match = re.search("(failed)|(error)|(warning)|(mistake)", status, re.I)
        if match:
           self.failedbuilds.append(collapsiblehtml)
        else:
           match = re.search("(passed)|(uxpassed)", status, re.I)
           if match:
               self.passedbuilds.append(collapsiblehtml)
           else:
               self.otherbuilds.append(collapsiblehtml)
    ...
    def _format_test(self, m):
        ...
        match = re.search("(failed)|(error)|(warning)|(mistake)", subunit_to_buildfarm_result(m.group(3)), re.I)
        if match:
           self.failedbuilds.append(collapsiblehtml)
        else:
           match = re.search("(passed)|(uxpassed)", subunit_to_buildfarm_result(m.group(3)), re.I)
           if match:
               self.passedbuilds.append(collapsiblehtml)
           else:
               self.otherbuilds.append(collapsiblehtml)
